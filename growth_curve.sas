...
proc nlin data=growth_data method=marquardt outest=param_est;
    ...
    model weight = A * exp(-exp(-B * (td - K))); /* Growth curve equation */
...
proc sql;
    ...
    select Animal, mean(weight) as mean_weight
    from growth_data
    group by Animal; /* Calculate mean weight per animal */
quit;
...
proc sql;
    ...
    group by a.Animal; /* Calculate R² for each animal */
quit;
...
proc export data=parameters
    outfile="D:\growth_parameters.xlsx"
    dbms=xlsx
